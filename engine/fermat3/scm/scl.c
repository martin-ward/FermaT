 ...
 * Copyright (C) 1990, 1991, 1992, 1993, 1994, 1995, 1997, 2005, 2006, 2013 Free Software Foundation, Inc.
 *
...
static double pmantexp2dbl P((SCM manstr, int expo));
     ...
     sizet ch;
{
  SCM fb, quo;
  sizet dp = ch;
  int e2, k, ndig = dbl_mant_dig;
  /* code from scm_dfloat_parts() */
  double mant = frexp(f, &e2);
#  ifdef DBL_MIN_EXP
  if (e2 < DBL_MIN_EXP)
    ndig -= DBL_MIN_EXP - e2;
#  endif
  e2 -= ndig;
/* if (scm_verbose > 1) printf("DBL_MIN_EXP = %d\n", DBL_MIN_EXP); -1021; */
  fb = dbl2big(ldexp(mant, ndig));
/* if (scm_verbose > 1) printf("mantissa = #x%s; e2 = %d\n", CHARS(number2string(fb, MAKINUM(16))), e2); */
  if (e2 > 0) {
    SCM num = scm_ash(fb, MAKINUM(e2)), den;
    /* try first with starved precision */
    k = ceil(log10(int2dbl(num)) - ndig*llog2);
    if (k < 0) k = 0;
    den = scm_intexpt(MAKINUM(10), MAKINUM(k));
    quo = scm_round_quotient(num, den);
    if (pmantexp2dbl(quo, k) != f) {
      k--; den = lquotient(den, MAKINUM(10));
      quo = scm_round_quotient(num, den);
    }
  } else {   /* e2 <= 0 */
    SCM num, den = scm_ash(MAKINUM(1L), MAKINUM(-e2));
    /* try first with starved precision */
    k = ceil(e2*llog2);
    num = product(fb, scm_intexpt(MAKINUM(10), MAKINUM(-k)));
    quo = scm_round_quotient(num, den);
    if (pmantexp2dbl(quo, k) != f) {
      k--; num = product(num, MAKINUM(10));
      quo = scm_round_quotient(num, den);
    }
  }
  a[ch++] = '.';
  ...
  a[dp] = a[dp+1]; a[++dp] = '.';
#  ifdef ENGNOT
  while ((dp+1 < ch) && (k+9999)%3) { a[dp] = a[dp+1]; a[++dp] = '.'; k--; }
#  endif	/* ENGNOT */
  while ('0'==a[--ch]); ch++;
  if (k != 0) {
    a[ch++] = 'e';
    return ch + ilong2str(k, 10, &a[ch]);
  } else return ch;
}
# else	/* ~BIGDIG */
     ...
     int point;
{
  ...
  return pmantexp2dbl(bmant, point);
}
     ...
     int point;
{
  ...
  if (point >= 0)
    return num2dbl(product(bmant, scm_intexpt(MAKINUM(10), MAKINUM(point))),
		   (char *)ARG1,
		   s_str2number);
  else {
    SCM scl = scm_intexpt(MAKINUM(10), MAKINUM(-point));
    int bex =			/* bex < 0 */
    ...
    return ldexp(num2dbl(quo, (char *)ARG1, s_str2number), bex);
  }
}
     ...
     SCM z1, z2;
{
 ...
 gencase:
  while(1) {
    if (0L==iz2) break;
    if (1L==iz2) {acc = product(acc, z1); break;}
    if (iz2 & 1) acc = product(acc, z1);
    z1 = product(z1, z1);
    iz2 >>= 1;
  }
 ret: return acc;
     ...
     SCM b;
{
  ...
  double ans = 0.0;
  if (INUMP(b)) return INUM(b);
  if (bex > 0) {
    quo = scm_round_quotient(num, scm_ash(MAKINUM(1L), MAKINUM(bex)));
  }
  /* quo may not be a bignum */
  if (INUMP(quo)) ans = ldexp(INUM(quo), bex);
  else {
    sizet i = NUMDIGS(quo);
